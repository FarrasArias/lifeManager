...
export function Card({ title, color = '#c8d8ff', children, collapsible, expanded, onToggle, action }) {
    ...
    <div style={{
      ...
      <div
        ...
        style={{
          ...
          borderBottom: isOpen ? '1px solid #1e2640' : 'none',
        }}
      >
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, color, letterSpacing: '0.02em' }}>{title}</h3>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {action}
          {collapsible && <span style={{ color: '#3a4560', fontSize: 14 }}>{isOpen ? '▾' : '▸'}</span>}
        </div>
      </div>
...
export function P({ children, style }) {
  return <p style={{ margin: '0 0 12px', fontSize: 14.5, color: '#a0b0cc', ...style }}>{children}</p>;
}

export function Ul({ children }) {
  return <ul style={{ margin: '0 0 12px', paddingLeft: 20, fontSize: 14.5, color: '#a0b0cc' }}>{children}</ul>;
}
...
export function Tag({ children, bg = '#1e2640' }) {
  return (
    <span style={{
      display: 'inline-block', background: bg, color: '#dde8ff',
