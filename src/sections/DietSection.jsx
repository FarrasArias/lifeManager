...
export default function DietSection() {
  ...
  return (
    <>
      <Card title="Diet Architecture" color="#dbb2c9">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 12 }}>
          {[
            ...
            <div key={i} style={{ background: '#2a2025', border: '1px solid #3a352e', padding: 14, borderRadius: 2 }}>
              <div style={{ fontSize: 12, color: '#8a8278', fontFamily: MONO }}>{d.label}</div>
              <div style={{ fontSize: 18, color: '#dbb2c9', fontWeight: 600, margin: '4px 0' }}>{d.value}</div>
              <div style={{ fontSize: 12, color: '#6a6358' }}>{d.sub}</div>
            </div>
          ))}
        </div>
      </Card>

      <Card title="Stool Quality Strategy" color="#dbb2c9">
      ...
      <Card title="Breakfast (Daily — Same)" color="#dbb2c9">
      ...
      <Card title="Lunch & Dinner — Meal Prep System" color="#dbb2c9" collapsible expanded={!!open.meals} onToggle={() => toggle('meals')}>
        ...
        <SectionTitle color="#dbb2c9">🍜 Dry Noodle Recipes</SectionTitle>
        {[
        ...
        <SectionTitle color="#dbb2c9">🍔 Lean Beef Burger</SectionTitle>
        ...
        <SectionTitle color="#dbb2c9">🍚 Asian Rice Bowls (Batch Rotation)</SectionTitle>
        {[
        ...
        <SectionTitle color="#dbb2c9">🥩 Non-Asian Options</SectionTitle>
        {[
      ...
      <Card title="Dinner / Evening Snack" color="#dbb2c9">
      ...
      <Card title="Ramen Day Integration" color="#dbb2c9">
      ...
      <Card title="Supplements Summary" color="#dbb2c9">
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #3a352e' }}>
                {['Supplement', 'Dose', 'When', 'Why'].map(h => (
                  <th key={h} style={{ textAlign: 'left', padding: '8px 10px', color: '#8a8278', fontWeight: 400 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderBottom: '1px solid #2a2520' }}>
                  <td style={{ padding: '8px 10px', color: '#dbb2c9' }}>{s.name}</td>
                  <td style={{ padding: '8px 10px', color: '#ccc5b9' }}>{s.dose}</td>
                  <td style={{ padding: '8px 10px', color: '#ccc5b9' }}>{s.when}</td>
                  <td style={{ padding: '8px 10px', color: '#8a8278', fontSize: 12 }}>{s.why}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function RecipeBlock({ name, desc }) {
  return (
    <div style={{ background: '#2a2025', padding: 14, borderRadius: 2, border: '1px solid #3a352e', marginBottom: 10 }}>
      <div style={{ fontWeight: 600, color: '#e8d5b7', marginBottom: 6 }}>{name}</div>
      <p style={{ margin: 0, fontSize: 14, color: '#ccc5b9', lineHeight: 1.6 }}>{desc}</p>
    </div>
  );
}
