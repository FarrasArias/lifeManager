...
export default function Overview() {
  return (
    <>
      <Card title="The Philosophy" color="#c8d8ff">
      ...
      <Card title="Priority Layers" color="#c8d8ff">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            { layer: 'Layer 1 — Foundation (Weeks 1–6)', bg: '#0e1828', items: ['Complete physio program for sciatica', 'Re-establish consistent sleep schedule', 'Lock in baseline diet', 'Core strengthening begins (isometrics only)', 'Maintain current gym routine at same weights'] },
            { layer: 'Layer 2 — Building (Weeks 7–16)', bg: '#0e0e28', items: ['Begin slow progressive overload in gym', 'Introduce gentle flexibility work (physio-approved)', 'Dial in meal prep system', 'Introduce light cardio (stationary bike)', 'Begin sleep transition from phone to audio-only'] },
            { layer: 'Layer 3 — Optimization (Weeks 17+)', bg: '#18081e', items: ['Push aesthetic goals', 'Reintroduce running/swimming gradually', 'Pursue deeper flexibility', 'Cut cycles in summer', 'Full autonomy over routine'] },
          ].map((l, i) => (
            <div key={i} style={{ background: l.bg, padding: '16px 18px', borderRadius: 2, border: '1px solid #1e2640' }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#c8d8ff', marginBottom: 8 }}>{l.layer}</div>
              <div style={{ fontSize: 13.5, color: '#a0b0cc', lineHeight: 1.7 }}>
      ...
      <Card title="Key Rules" color="#c8d8ff">
        <Ul>
